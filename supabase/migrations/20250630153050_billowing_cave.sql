/*
  # Subscription Integration with Supabase - Safe Migration

  1. Tables
    - Ensure `user_subscriptions` table exists with all required columns
    - Add missing columns if they don't exist

  2. Security
    - Safely create RLS policies only if they don't exist
    - Enable RLS on table

  3. Functions
    - Create or replace subscription-related functions
    - Add usage tracking functions
*/

-- Ensure user_subscriptions table exists with all columns
DO $$
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE table_name = 'user_subscriptions' AND column_name = 'revenue_cat_user_id'
  ) THEN
    ALTER TABLE user_subscriptions ADD COLUMN revenue_cat_user_id text;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'user_subscriptions' AND column_name = 'is_subscribed'
  ) THEN
    ALTER TABLE user_subscriptions ADD COLUMN is_subscribed boolean DEFAULT false;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'user_subscriptions' AND column_name = 'subscription_type'
  ) THEN
    ALTER TABLE user_subscriptions ADD COLUMN subscription_type text;
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE user_subscriptions ADD COLUMN expires_at timestamptz;
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'user_subscriptions' AND column_name = 'created_at'
  ) THEN
...
BEGIN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_subscriptions' 
    AND policyname = 'Users can insert own subscription data'
  ) THEN
  ...
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'user_subscriptions' 
    AND policyname = 'Users can update own subscription data'
  ) THEN
...
-- Add usage tracking column to stories table if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
